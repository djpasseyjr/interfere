...
def run_forecasts(
    ...
    method_preds = method_progress["X_do_preds"]

    start = len(method_preds)
    end = len(Xs)
    ...
    for i in range(start, end):
        ...
        # Compute forecasts (potentially more than one).
        X_do_preds, best_params = interfere.benchmarking.forecast_intervention(
            Xs[i], X_dos[i], t, intervention, **method_args, best_params=best_params)
        
        # Save predictions and optimized params.
        method_preds.append(X_do_preds)

        # This resets best_params to none so hyper parameter tuning is done
        # every iteration within forecast_intervention(). Otherwise it is
...
def args_are_equal(arg1, arg2):
    ...
    return arg1.__name__ == arg2.__name__
