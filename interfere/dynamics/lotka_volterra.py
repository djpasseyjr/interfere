...
class LotkaVolterra(OrdinaryDifferentialEquation):
    """Class for simulating Lotka Volterra dynamics.
    ...
    def __init__(
        ...
        """Initializes class for simulating Lotka Volterra dynamics.
        ...
        * https://github.com/netsiphd/netrd/blob/master/netrd/dynamics/lotka_volterra.py
        ...
        if any(
            ...
            raise ValueError(
                "Parameters for Lotka Volterra must have the same "
        ...
        if not isinstance(self, LotkaVolterraSDE):
            # Check if the user still supplied a stochastic argument.
            if (sigma is not None) and (sigma != 0):
                raise ValueError(
                    "Type interfere.dynamics.LotkaVolterra cannot be stochastic."
                    " Use interfere.dynamics.LotkaVolterraSDE or set sigma=None "
    ...
    def dXdt(self, x: np.ndarray, t: Optional[float] = None):
        """Computes derivative of a generalized Lotka Volterra model.
...
class LotkaVolterraSDE(StochasticDifferentialEquation, LotkaVolterra):
    def __init__(
        self,
        growth_rates: np.ndarray,
        capacities: np.ndarray,
        ...
        * https://en.wikipedia.org/wiki/Competitive_Lotka%E2%80%93Volterra_equations
        ...
        Args:
            ...
            measurement_noise_std (ndarray): None, or a vector with shape (n,)
                ...
                will be added to x1 and x2 respectively at each point in time.
        """
        # The following line actually uses the LotkaVolterra.__init__()
