    ...
    def _simulate(
        ...
        **kwargs,
        ...
        if self.tsteps_btw_obs == 1:
            ...
            warn(
                "Intervention is ambiguous for downsampled systems. Current "
            ...
            for ti in t[1:]:
                ...
                frozen_times = np.hstack(
                    [[t_prev], ti * np.ones(self.tsteps_btw_obs + 1)]
                )
...
def coupled_map_1dlattice_spatiotemp_chaos(
    ...
    """
    return stochastic_coupled_map_1dlattice(
        dim, 2.0, 0.3, sigma=sigma, measurement_noise_std=measurement_noise_std
    )
