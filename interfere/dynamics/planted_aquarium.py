...
class PlantedTankNitrogenCycle(StochasticDifferentialEquation):
    ...
    def __init__(
        ...
        nitirite_bact_eff_const = np.arctan(
            (nitrite_bact_eff_y_max - nitrite_bact_eff_y_min)
            * t_min
            / reference_time_v
            + nitrite_bact_eff_y_min
        )

        nitrate_bact_eff_const = np.arctan(
            (nitrate_bact_eff_z_max - nitrate_bact_eff_z_min)
            * t_min
            / reference_time_v
            + nitrate_bact_eff_z_min
        )
        ...
        # p1 in paper.
        self.ammon_prod_eff_p1 = lambda t: ammonia_eff_gamma * (
            np.arctan(
                (ammonia_eff_x_max - ammonia_eff_x_min) * t / reference_time_v
                + ammonia_eff_x_min
            )
            - ammonia_eff_const
        )
        ...
        # mu1 in the paper.
        self.nitrite_bacteria_effic_mu1 = lambda t: nitrite_bact_eff_gamma1 * (
            np.arctan(
                (nitrite_bact_eff_y_max - nitrite_bact_eff_y_min)
                * t
                / reference_time_v
            ...
            - nitirite_bact_eff_const
        )

        # mu2 in paper.
        self.nitrate_bacteria_effic_mu2 = lambda t: nitrate_bact_eff_gamma2 * (
            np.arctan(
                (nitrate_bact_eff_z_max - nitrate_bact_eff_z_min)
                * t
                / reference_time_v
    ...
    def drift(self, x: np.ndarray, t: float) -> np.ndarray:
        ...
        d_ammonium_dt = (
            self.ammon_prod_eff_p1(t) * (1 - self.ammonia_limit_q1 * ammonia)
            - self.nitrite_bacteria_effic_mu1(t) * ammonia**2 * oxy**3
        )

        d_oxy_dt = (
            self.oxy_prod_p2 * (1 - self.oxy_limit_q2 * oxy)
            - self.nitrite_bacteria_effic_mu1(t) * ammonia**2 * oxy**3
            - self.nitrate_bacteria_effic_mu2(t) * oxy**0.5 * nitrite
        )

        d_nitrite_dt = (
            self.nitrite_bacteria_effic_mu1(t) * ammonia**2 * oxy**3
            - self.nitrate_bacteria_effic_mu2(t) * oxy**0.5 * nitrite
            - self.nitrite_cons_p3 * nitrite
        )

        d_nitrate_dt = (
            self.nitrate_bacteria_effic_mu2(t) * oxy**0.5 * nitrite
            - self.nitrate_cons_p4 * nitrate
        )
