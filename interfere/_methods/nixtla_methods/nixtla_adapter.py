...
from neuralforecast.common._base_model import (
    BaseModel as NeuralForecastBaseModel,
)
import numpy as np
import pandas as pd
...
class NixtlaAdapter(ForecastMethod):
    ...
    def __init__(
        ...
        logging.getLogger("pytorch_lightning.accelerators.cuda").setLevel(
            logging.WARNING
        )
        logging.getLogger("lightning_fabric.utilities.seed").setLevel(
            logging.WARNING
        )
        logging.getLogger(
    ...
    def neuralforecast_predict(
        ...
        futr_times = np.hstack(
            [discrete_t, np.arange(1, n_extra_preds + 1) + discrete_t[-1]]
        )

        if prediction_exog is None:
            prediction_exog = np.full(
                (len(discrete_t), len(exog_state_ids)), np.inf
            )
        ...
        _, endog_pred, _ = to_interfere_arrays(
            df, unique_ids=[id for id in df.unique_id.unique()]
        )
    ...
    def statsforecast_predict(
        ...
        _, endog_pred, _ = to_interfere_arrays(
            pred_df, unique_ids=[id for id in pred_df.unique_id.unique()]
        )
...
def to_nixtla_df(
    ...
    if (endog_states is None) and (unique_ids is None):
        raise ValueError(
            "unique ids cannot be determined unless one of "
    ...
    if exog_states is None:
        if not ((exog_state_ids is None) or (exog_state_ids == [])):
            raise ValueError(
                "Exogengous state ids passed without "
        ...
        data_chunks = [np.hstack([x, exog_states]) for x in data_chunks]
    ...
    nf_data = pd.DataFrame(np.vstack(data_chunks), columns=col_names)
...
def to_interfere_arrays(
    ...
    if unique_ids is None:
        ...
        if len(set(unique_ids).symmetric_difference(dflt_unique_ids)) > 0:
            raise ValueError(
                "No value passed to `unique_id` argument but "
    ...
    if exog_state_ids is None:
        exog_state_ids = [
            col_name
            for col_name in nixtla_df.columns
            if col_name not in ["unique_id", "ds", "y"]
        ]

    # Extract endog variables.
    endog_states = np.vstack(
        [nixtla_df[nixtla_df.unique_id == id].y for id in unique_ids]
    ).T
