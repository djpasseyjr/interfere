...
def belozyorov_timeseries(tsteps=100, n_do=20):
    ...
    X_do = model.simulate(
        forecast_times, X0_do, intervention=intervention, rng=rng
    )
...
def make_optuna_training_data():
    ...
    # Forecast data.
    forecast_states = model.simulate(t=forecast_t, prior_states=train_states)

    # Intervention data.
    intervention = interfere.PerfectIntervention(0, 1.0)
    interv_states = model.simulate(
        t=forecast_t, prior_states=train_states, intervention=intervention
    )
...
class TestFitPredict:
    ...
    def test_predict(
        ...
        endo_pred = method.predict(
            forecast_times, prior_endog_states, prior_t=prior_t
        )
    ...
    def test_predict_arbitrary_initial(
        ...
        assert np.allclose(
            random_initial[-1, :], endog_pred[0, :], atol=0.1
        ), (
...
class TestPredictErrors:
    ...
    def test_infer_prior_t_warn(
        self, method_type: Type[interfere.ForecastMethod]
    ):
    ...
    def test_equally_spaced_warn(
        ...
        with pytest.raises(
            ...
            method.predict(
                np.hstack([t, [t[-1] + np.pi]]),
                prior_endog_states[-1, :],
                prior_t=None,
            )

    def test_last_prior_t_equals_first_t(
        self, method_type: Type[interfere.ForecastMethod]
    ):
    ...
    def test_prior_t_match_prior_endog(
        self, method_type: Type[interfere.ForecastMethod]
    ):
    ...
    def test_window_size_warn(
        ...
        with pytest.warns(
            UserWarning,
            match=str(type(method).__name__) + " has window size "
    ...
    def test_window_size_warn_exog(
        self, method_type: Type[interfere.ForecastMethod]
    ):
        """Test that predict raises warning when not enough historic data is
        passed.
    ...
    def test_prior_t_infer_warning(
        self, method_type: Type[interfere.ForecastMethod]
    ):
...
def test_continuous_predict_arbitrary_initial(
    ...
    rand_prior_states = np.vstack(
        [np.sin(rand_prior_t), np.cos(rand_prior_t)]
    ).T

    endo_pred = method.predict(pred_t, rand_prior_states, prior_t=rand_prior_t)
...
def test_optimize_method_exog_response(
    ...
    (endo_train_states, exog_train_states) = intervention.split_exog(
        train_states
    )
    ...
    interv_endo_pred = method.predict(
        forecast_t,
        prior_endog_states=endo_train_states,
        prior_exog_states=exog_train_states,
        prior_t=train_t,
        prediction_exog=exog_pred,
    )
    interv_pred = intervention.combine_exog(interv_endo_pred, exog_pred)

    control_error = interfere.metrics.RootMeanStandardizedSquaredError()(
        train_states, forecast_states, interv_pred, intervention
    )

    interv_error = interfere.metrics.RootMeanStandardizedSquaredError()(
        train_states, interv_states, interv_pred, intervention
    )
...
def test_sindy_refit():
    ...
    for diff in interfere._methods.sindy.SINDY_DIFF_LIST:
        for lib in interfere._methods.sindy.SINDY_LIB_LIST:
            ...
            method.fit(t, X)
            pred = method.predict(np.arange(t[-1], t[-1] + 5), X, prior_t=t)
