...
def test_perfect_intervention():
    ...
    assert np.all(g(np.array([0.1, 0.2, 0.0]), 0) == np.array([0.1, 0.2, 0.3]))
    X = rng.random((10, 5))
    X_en, X_ex = g.split_exog(X)
    assert np.all(X_en == np.hstack([X[:, :2], X[:, 3:]]))
    assert np.all(X_ex == X[:, 2:3])

    g = interfere.PerfectIntervention([0, 1], [0, 0])
    assert np.all(g(np.array([0.1, 0.2, 0.0]), 0.1) == np.zeros(3))

    assert np.all(g.eval_at_times(np.array([0, 1, 2])) == np.zeros((3, 2)))
    X = rng.random((10, 5))
    X_en, X_ex = g.split_exog(X)
...
def test_split_exog():
    ...
    with pytest.raises(ValueError, match=(f"Array must be two dimensional.")):
        interv.split_exog(np.ones(10))

    with pytest.raises(
        ValueError, match=(f"Some intervention indexes are too big.")
    ):
...
def test_combine_exog():
    ...
    with pytest.raises(
        ValueError,
        match=("Exogenous states was None but intervention expects"),
    ):
    ...
    with pytest.raises(
        ...
        interv2 = interfere.PerfectIntervention([1, 2], [0.5, 0.5])
    ...
    with pytest.raises(
        ValueError,
        match=("Wrong number of exogenous signals passed to intervention."),
    ):
...
def test_eval_at_times():
    ...
    X2 = model.simulate(
        t, x0, intervention=interfere.PerfectIntervention(0, 5)
    )
